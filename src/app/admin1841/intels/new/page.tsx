...
export default function NewIntel() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = await (supabase as any).from("intels").insert(formData);

        if (error) {
            alert("Save failed: " + error.message);
        } else {
            router.push("/admin1841/intels");
        }
        setLoading(false);
    };
        ...
        <div className="min-h-screen bg-black text-white pt-24 pb-12 px-6">
            <div className="max-w-4xl mx-auto">
                <header className="mb-10">
                    <h1 className="text-3xl font-orbitron font-bold uppercase tracking-tight">Author Intel</h1>
                    <a href="/admin1841/intels" className="text-slate-400 hover:text-white font-rajdhani">← Cancel Operation</a>
                </header>
                ...
                <form onSubmit={handleSubmit} className="space-y-8 glass-panel p-8 border border-white/10">
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    required
                                    placeholder="e.g. Robotics, Cyber Security"
                                    className="w-full bg-white/5 border border-white/10 p-3 font-rajdhani focus:border-white/40 outline-none"
                                    value={formData.tag}
                        ...
                        <div className="space-y-4">
                            <label className="block text-slate-500 font-rajdhani text-sm uppercase mb-2">Visual Data (Image)</label>
                    ...
                    <div>
                        <label className="block text-slate-500 font-rajdhani text-sm uppercase mb-2">Mission Intelligence (Content)</label>
                        <textarea
                            required
                            rows={10}
                            className="w-full bg-white/5 border border-white/10 p-3 font-rajdhani focus:border-white/40 outline-none"
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-white text-black font-orbitron font-bold tracking-widest hover:bg-slate-200 transition-all disabled:opacity-50"
                    >
                        {loading ? "COMMITTING TO DATABASE..." : "DECALRE INTEL"}
                    </button>
                </form>
            </div>
        </div>
